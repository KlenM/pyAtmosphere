...
class Measures:
  def __init__(
    ...
    self.name = name or (operations[0].__name__ if len(operations) == 1 and operations[0].__name__ != "<lambda>" else "")
...
class Simulation:
  ...
  def add_measures(self, measures):
    ...
    self.measures[measures.channel][measures.time][measures.measure_type][measures.operations].append(measures)
    ...
    for measures in self.flattened_measures():        
      ...
      if measures.time: 
        if measures.measure_type == "propagation":
          empty_data = [[None for _ in range(len(measures.channel.path.positions) + 1)] for _ in measures.time]
        else:
          empty_data = [None for _ in measures.time]
      elif measures.measure_type == "propagation":
        empty_data = [None for _ in range(len(measures.channel.path.positions) + 1)]
  ...
  def process_operations(self, output, operations_measures, time_id, propagation_id=None):
    for operations, measures_list in operations_measures.items():
      ...
      for operation in operations:
        measures_output = operation(measures.channel, output=measures_output)
      ...
      for time, time_measures in channel_measures.items():
        ...
        for time_id, time_value in enumerate(time):
          for propagation_id, (propagation_result, phase_screen) in enumerate(channel.generator(pupil=False, shift=(0, time_value or 0), store_output=True, wind=True)):
            self.process_operations(propagation_result, time_measures.get("propagation", {}), time_id, propagation_id)
            if propagation_id == 0:
              self.process_operations(phase_screen, time_measures.get("phase_screen", {}), time_id)
          self.process_operations(channel.output, time_measures.get("atmosphere", {}), time_id)
          self.process_operations(channel.output, time_measures.get("propagation", {}), time_id, -1)
          if channel.pupil:
            self.process_operations(channel.pupil.output(channel.output), time_measures.get("pupil", {}), time_id)
    for measures in self.flattened_measures():
  ...
  def run(self, *args, plot_step: int = None, save_step: int = None, **kwargs):
    try:
      ...
      while not self.is_measures_done():
        self.iter()
        iteration += 1
        self.process_output(iteration, plot_step=plot_step, save_step=save_step)
