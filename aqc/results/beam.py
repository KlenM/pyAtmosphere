...
class BeamResult(Result):
  ...
  def mean_x2_r(self, channel, output):
    r0 = np.sqrt(self.measures[0].iteration_data**2 + self.measures[1].iteration_data**2)
    cosXi = self.measures[0].iteration_data / r0
    sinXi = self.measures[1].iteration_data / r0
    rotated_x2_r = (channel.grid.get_x() * cosXi + ((-1) * channel.grid.get_y()) * sinXi)**2
...
class BeamPropagationResult(Result):  
  def __init__(self, channel, **kwargs):
    measures = [Measures(channel, "propagation", mean_r), Measures(channel, "propagation", mean_r2)]
    super().__init__(channel, measures, **kwargs)
  ...
  def plot_output(self):
    ...
    axs[0].plot(self.positions, np.sqrt(self.bw2.mean(axis=0)), label="Simulated")
    axs[0].plot(self.positions, self.bw_theoretical, label="Theory")
    axs[0].plot(np.nan, np.nan, label=f"Iterations: {len(self.measures[0])}", alpha=0)
    axs[0].legend()

    axs[1].set_ylabel(r"Long term $W_{LT}$, m")
    axs[1].plot(self.positions, np.sqrt(self.lt2.mean(axis=0)), label="Simulated")
    axs[1].plot(self.positions, self.lt_theoretical, label="Theory")
    axs[1].plot(np.nan, np.nan, label=f"Iterations: {len(self.measures[0])}", alpha=0)
    axs[1].legend()
    ...
    axs[2].plot(np.nan, np.nan, label=f"Iterations: {len(self.measures[0])}", alpha=0)
    axs[2].legend()
    plt.show()