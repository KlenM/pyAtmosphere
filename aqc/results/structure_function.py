...
class StructureFunctionResult(Result):
  def __init__(self, channel, *args, **kwargs):
    measures = [Measures(channel, "phase_screen", calculate_structure_function)]
  ...
  def init_theoretical(self):
    ...
    self.get_theoretical = channel.path.phase_screen.model.sf_phi(self.r, k, channel.path.phase_screen.thickness)
    self.get_numerical_theoretical = channel.path.phase_screen.model.sf_phi_numeric(self.r, k, channel.path.phase_screen.thickness)
  ...
  def plot_output(self):
    ...
    plt.plot(self.r, self.get_numerical_theoretical, label="Theoretical numerical")
    plt.plot(np.nan, np.nan, label=f"Iterations: {len(self.measures[0])}", alpha=0)
    plt.legend()
    plt.show()
