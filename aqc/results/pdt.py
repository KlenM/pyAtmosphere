...
class PDTResult(Result):
  def __init__(self, channel, pupils: list = None, **kwargs):
    ...
    measures = kwargs.pop("measures", [
        Measures(channel, "atmosphere", partial(self.append_pupil, pupil), eta, name=f"{pupil.radius}") 
  ...
  def plot_output(self):
    if len(self.pupils) == 1:
      plt.hist(self.measures[0].data, label=f"Count: {len(self.measures[0])}", bins=200, range=(0, 1))
      plt.legend()
      plt.show()
    else:
      n_x = 3
...
class TrackedPDTResult(PDTResult):
  def __init__(self, channel, pupils: list = None, **kwargs):
    ...
    beam_measures = [Measures(channel, "atmosphere", mean_x), Measures(channel, "atmosphere", mean_y)]
    pdt_measures = [
        Measures(channel, "atmosphere", self.set_pupil_position, partial(self.append_pupil, pupil), eta, name=f"{pupil.radius}") 
      for pupil in pupils]
    super().__init__(channel, pupils=pupils, measures=beam_measures + pdt_measures, **kwargs)
  
  def set_pupil_position(self, channel, output):
    self.pupil_shift = (self.measures[0].iteration_data, self.measures[1].iteration_data)
    return output
