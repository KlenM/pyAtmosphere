...
class SIResult(Result):
  ...
  def set_theoretical_functions(self, *theoretical_functions):
    ...
    self.theoretical_si = [theoretical_function(self.positions, self.channel.path.phase_screen.model, self.channel.source) for theoretical_function in theoretical_functions]
  ...
  def plot_output(self):
    plt.plot(self.positions, self.si, label=r"On-axis SI $\sigma_I$, m", **self.plot_kwargs)
    for i, theoretical_function in enumerate(self.theoretical_functions):
      plt.plot(self.positions, self.theoretical_si[i], label=f"Theoretical on-axis SI: {theoretical_function.__name__}")
