...
def bw_pdt(eta, eta_0, R, l, bw2):
    with warnings.catch_warnings():
        warnings.filterwarnings('ignore', message=r"divide by zero encountered in", category=RuntimeWarning)
        warnings.filterwarnings('ignore', message=r"invalid value encountered in (power|multiply)", category=RuntimeWarning)
        pdt = R**2 / (bw2 * eta * l) * np.log(eta_0 / eta)**(2 / l - 1) * np.exp(-(R**2 / (2 * bw2)) * np.log(eta_0 / eta)**(2 / l))
        pdt[np.isnan(pdt)] = 0
        return pdt
...
def bayesian_pdt(eta, eta_mean, eta2_mean, a, st2, bw2, scale_R=None, shape_l=None, r0_size=2000):
    ...
    for r0 in np.random.rayleigh(bw, size=r0_size):
      mu_r0 = -np.log(eta0**2 / np.sqrt(zeta02)) + (abs(r0) / scale_R)**shape_l
      lognorm_model = lognorm(sigma_r0, scale=np.exp(-mu_r0))
      total_probability_model += lognorm_model.pdf(eta) / lognorm_model.cdf(1)
...
def elliptic_beam_numerical_transmission(beam_params: dict, pupil_radiuses, resolution=2**8, is_tracked=False):
    ...
    for mean_x, mean_y, mean_x2, mean_y2, mean_xy in zip(mean_x, mean_y, mean_x2, mean_y2, mean_xy):
        ...
        intensity = np.sqrt(2 / np.pi / np.sqrt(detS)) * cupy.exp(-(Syy * X**2 - 2 * Sxy * X * Y + Sxx * Y**2) / detS)
        for i, pupil in enumerate(pupils):
            shift = (0, 0) if not is_tracked else (mean_x, mean_y)
            model_eta[i].append(eta_measure(DummyChannel, output=intensity * pupil.get_pupil(shift=shift)))
    return model_eta
