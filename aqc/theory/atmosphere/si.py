...
def get_SI_andrews_weak_kolmogorov(length, model, gaussian_beam):
    ...
    if np.isfinite(gaussian_beam.F0):
        raise ValueError(
            "The function has been implemented only for collimated beam.")

    SR2 = get_rytov2(model.Cn2, gaussian_beam.k, length)
    r0_s = get_r0s(model.Cn2, length, gaussian_beam.k)
    c_r = 2 * np.pi
    r_cw2_mean = 2.42 * model.Cn2 * length**3 / \
        gaussian_beam.w0**(1/3) * hyp2f1(1/3, 1, 4, length / gaussian_beam.F0)
    Spe = r_cw2_mean * (1 - (c_r**2 * gaussian_beam.w0**2 / r0_s) /
                        (1 + c_r**2 * gaussian_beam.w0**2 / r0_s))**(1 / 6)
...
def get_SI_andrews_strong_zeroscale(length, model, gaussian_beam, debug=False):
    ...
    """
    SR2 = get_rytov2(model.Cn2, gaussian_beam.k, length)
    SB2 = 3.86 * SR2 * np.real(1j**(5/6) * hyp2f1(-5/6, 11/6, 17/6, (1 - gaussian_beam.get_theta(
        length)) + 1j * gaussian_beam.get_Lambda(length)) - 11 / 16 * gaussian_beam.get_Lambda(length)**(5/6))

    ### !!! ###
    ### SB**(12 / 5) == SB2**(6 / 5)
    ### !!! ###
    SlnX2 = 0.49 * SB2 / \
        (1 + 0.56 * (1 + gaussian_beam.get_theta(length)) * SB2**(6 / 5))**(7 / 6)
...
def get_SI_andrews_strong(length, model, gaussian_beam, debug=False):
    ...
    SR2 = get_rytov2(model.Cn2, gaussian_beam.k, length)
    phi_1 = np.arctan(2 * gaussian_beam.get_Lambda(length) /
                      (1 + 2 * gaussian_beam.get_theta(length)))
    phi_2 = np.arctan((1 + 2 * gaussian_beam.get_theta(length))
                      * Ql / (3 + 2 * gaussian_beam.get_Lambda(length) * Ql))

    SG2_1 = 0.4 * ((1 + 2 * gaussian_beam.get_theta(length))**2 + (2 * gaussian_beam.get_Lambda(length) + 3 / Ql) **
                   2)**(11 / 12) / ((1 + 2 * gaussian_beam.get_theta(length))**2 + 4 * gaussian_beam.get_Lambda(length)**2)**(1 / 2)
    SG2_2 = np.sin(11 / 6 * phi_2 + phi_1)
    SG2_3 = 2.61 / ((1 + 2 * gaussian_beam.get_theta(length))**2 * Ql**2 + (3 + 2 *
                    gaussian_beam.get_Lambda(length) * Ql)**2)**(1 / 4) * np.sin(4 / 3 * phi_2 + phi_1)
    SG2_4 = 0.52 / ((1 + 2 * gaussian_beam.get_theta(length))**2 * Ql**2 + (3 + 2 *
                    gaussian_beam.get_Lambda(length) * Ql)**2)**(7 / 24) * np.sin(5 / 4 * phi_2 + phi_1)
    SG2_5 = 13.40 * gaussian_beam.get_Lambda(length) / Ql**(11 / 6) / (
        (1 + 2 * gaussian_beam.get_theta(length))**2 + 4 * gaussian_beam.get_Lambda(length)**2)
    SG2_6 = 11 / 6 * ((1 + 0.31 * gaussian_beam.get_Lambda(length) * Ql)**(5 / 6) + (1.1 * (1 + 0.27 * gaussian_beam.get_Lambda(
        length) * Ql)**(1 / 3)) - (0.19 * (1 + 0.24 * gaussian_beam.get_Lambda(length) * Ql)**(1 / 4))) / Ql**(5 / 6)
    SG2 = 3.86 * SR2 * (SG2_1 * (SG2_2 + SG2_3 - SG2_4) - SG2_5 - SG2_6)

    eta_X_1 = 0.38 / (1 - 3.21 * (1 - gaussian_beam.get_theta(length)
                                  ) + 5.29 * (1 - gaussian_beam.get_theta(length))**2)
    eta_X_2 = 0.47 * SR2 * Ql**(1 / 6) * ((1 / 3 - 1 / 2 * (1 - gaussian_beam.get_theta(length)) + 1 / 5 * (
        1 - gaussian_beam.get_theta(length))**2) / (1 + 2.2 * (1 - gaussian_beam.get_theta(length))))**(6 / 7)
    eta_X = (eta_X_1 + eta_X_2)**(-1)
    eta_X0 = eta_X * Q0 / (eta_X + Q0)

    SlnX2l0_1 = 1 / 3 - 1 / 2 * \
        (1 - gaussian_beam.get_theta(length)) + 1 / \
        5 * (1 - gaussian_beam.get_theta(length))**2
